...
export function startPolling(searchCache, seatsApiKey) {
  ...
  setInterval(() => runPollCycle(searchCache, seatsApiKey), intervalMs);
}

async function runPollCycle(searchCache, seatsApiKey) {
  const now = new Date().toISOString().slice(0, 10);

  // Auto-disable alerts whose date_to has passed
  db.prepare(`UPDATE alerts SET enabled = 0 WHERE enabled = 1 AND date_to IS NOT NULL AND date_to < ?`).run(now);
  ...
  for (const [, groupAlerts] of groups) {
    ...
    for (const alert of groupAlerts) {
      if (rateLimited) break;
      try {
        const result = await evaluateAlert(alert.id, searchCache, seatsApiKey);
