...
router.post('/', (req, res) => {
  ...
  const activeCount = db.prepare(`SELECT COUNT(*) AS n FROM alerts WHERE enabled = 1`).get().n;
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE alerts SET ${setClauses} WHERE id = @id`).run({ ...updates, id });
...
router.post('/:id/run', async (req, res) => {
  ...
  try {
    const result = await evaluateAlert(id, searchCache, seatsApiKey);
...
router.post('/matches/:id/dismiss', (req, res) => {
  const id = parseInt(req.params.id, 10);
  const r = db.prepare(`UPDATE alert_matches SET status = 'dismissed' WHERE id = ?`).run(id);
